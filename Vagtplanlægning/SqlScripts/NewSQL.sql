...
-- Right now it’s NOT NULL, but sometimes a shift may not have a substitute. Consider changing it to NULL by default.
    ...
    ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_shift_substituted FOREIGN KEY (substitutedId) REFERENCES Substituteds(substitutedId)
    ON DELETE CASCADE ON UPDATE CASCADE,
...
-- Create indexes --------------------------------------------------------------
-- =========================
CREATE INDEX idx_shift_date_employee
-- (run once on fresh DB; remove if you re-run script many times)
    ON ListOfShift(dateOfShift, employeeId);

CREATE INDEX idx_shift_date_substituted
-- (run once on fresh DB; remove if you re-run script many times)
...
CREATE UNIQUE INDEX idx_payroll_employee_period
    ON WorkHoursInMonths(employeeId, periodStart, periodEnd);
...
SELECT
    ...
    COALESCE(CONCAT(su_e.firstName, ' ', su_e.lastName), 'Ingen employee') AS Substituted,
    su.hasSubstituted
FROM ListOfShift s
         JOIN Employees e ON s.employeeId = e.employeeId
         JOIN Routes r ON s.routeId = r.id
...
BEGIN
    ...
    IF pYear IS NOT NULL AND pMonth IS NOT NULL THEN
...
SELECT
    ...
    pYear AS year,
        ...
        COALESCE(SUM(
            CASE 
                WHEN s.substitutedId = e.employeeId OR s.employeeId = e.employeeId THEN s.totalHours
                ELSE 0
            END
        ), 0) AS totalMonthlyHours,
        CASE
            WHEN SUM(CASE WHEN s.substitutedId = e.employeeId THEN 1 ELSE 0 END) > 0
            THEN TRUE ELSE FALSE
END AS hasSubstituted
    FROM Employees e
    LEFT JOIN ListOfShift s
        ON (s.employeeId = e.employeeId OR s.substitutedId = e.employeeId)
       AND (vPeriodStart IS NULL OR s.dateOfShift BETWEEN vPeriodStart AND vPeriodEnd)
    WHERE pEmployeeId IS NULL OR e.employeeId = pEmployeeId
...
DELIMITER $$
CREATE PROCEDURE GetRoutes (IN pRouteId INT)
BEGIN
SELECT id AS routeId, routeNumber
FROM Routes
WHERE pRouteId IS NULL OR id = pRouteId
...
DELIMITER $$
CREATE PROCEDURE GetBicycles (IN pBicycleId INT)
BEGIN
SELECT id AS bicycleId, bicycleNumber, inOperate
FROM Bicycles
WHERE pBicycleId IS NULL OR id = pBicycleId
...
DELIMITER $$
CREATE PROCEDURE GetEmployees (IN pEmployeeId INT)
BEGIN
SELECT
    employeeId,
    firstName,
    ...
    experienceLevel
FROM Employees
WHERE pEmployeeId IS NULL OR employeeId = pEmployeeId
...
BEGIN
INSERT INTO Employees (firstName, lastName, address, phone, email, experienceLevel)
VALUES (pFirstName, pLastName, pAddress, pPhone, pEmail, COALESCE(pExperienceLevel, 1));
END$$
DELIMITER ;
...
VALUES ((SELECT IFNULL(MAX(bicycleNumber),0)+1 FROM Bicycles), DEFAULT);
END$$
DELIMITER ;
...
VALUES ((SELECT IFNULL(MAX(routeNumber),0)+1 FROM Route));
END$$
DELIMITER ;
...
INSERT INTO ListOfShift (
    dateOfShift, employeeId, bicycleId, routeId, startTime, substitutedId
)
VALUES (
           pDateOfShift, pEmployeeId, pBicycleId, pRouteId, pStartTime, pSubstitutedId
       );
END$$
DELIMITER ;
...
-- =========================
DELIMITER $$
CREATE EVENT ev_generateWorkHoursInMonths
-- event: monthly rollup ------------------------------------------------------
...
STARTS '2025-09-25 23:50:00'
DO
BEGIN
INSERT INTO WorkHoursInMonths (
    employeeId, payrollYear, payrollMonth, periodStart, periodEnd, totalHours, hasSubstituted
)
SELECT
    e.employeeId,
    ...
    DATE_SUB(DATE_FORMAT(CURDATE(), '%Y-%m-26'), INTERVAL 1 MONTH) AS periodStart,
    ...
    THEN TIME_TO_SEC(TIMEDIFF(s.endTime, s.startTime)) / 3600
    ELSE 0
    END
    ), 0) AS totalHours,
...
END AS hasSubstituted
    ...
    FROM Employees e
    LEFT JOIN ListOfShift s 
        ON (e.employeeId = s.employeeId OR e.employeeId = s.substitutedId)
       AND s.dateOfShift BETWEEN DATE_SUB(DATE_FORMAT(CURDATE(), '%Y-%m-26'), INTERVAL 1 MONTH)
                              AND DATE_FORMAT(CURDATE(), '%Y-%m-25')
    LEFT JOIN Substituteds sub
        ON s.substitutedId = sub.substitutedId
